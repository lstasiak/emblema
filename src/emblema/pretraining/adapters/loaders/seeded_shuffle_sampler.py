...
class SeededShuffleSampler(Sampler[int]):
    ...
    Each position is ranked by a digest of itself under the seed and the epoch, and the order is
    that ranking. Two runs of the same seed see the same windows in the same order in every epoch,
    on any machine and under any version of torch, numpy or Python — none of their generators is
    involved. That matters because a run is prepared on one machine and executed on another.
    ...
    part of asking for an epoch's batches.
    """
    ...
    def _rank(self, position: int) -> bytes:
        return hashlib.blake2b(
            f"{self._seed}:{self._epoch}:{position}".encode(), digest_size=16
        ).digest()
