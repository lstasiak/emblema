...
class ChannelAggregatedFeatures:
    """Reads a window as its per-channel statistics summarised across whichever channels it holds.
    ...
    def names(self) -> tuple[str, ...]:
        ...
        return tuple(
            f"{across}_{name}" for name in WindowStatistics.NAMES for across in self.ACROSS
    ...
    def of(self, windows: Sequence[TokenWindow]) -> NDArray[np.float64]:
        ...
        for index, window in enumerate(windows):
            ...
            rows[index] = np.concatenate(
                (self._across(statistics.rows), self._of_window(window, statistics))
            )
        return rows

    @staticmethod
    def _across(rows: NDArray[np.float64]) -> NDArray[np.float64]:
        """Each statistic summarised over the channels that have one, ignoring those that do not.

        A statistic no channel of the window supports — a slope where every channel was observed
        once — stays absent rather than becoming a number, for the same reason it was absent per
        channel.
        """
        known = ~np.isnan(rows)
