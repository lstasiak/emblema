...
from emblema.catalog.domain.tokenisation.split_policy import NamedSplit, SeededSplit, SubsetSplit
...
def test_holding_out_a_subset_takes_every_unit_read_from_it_and_records_no_seed() -> None:
    policy = SubsetSplit("set-b")
...
def test_a_subset_no_unit_was_read_from_is_refused_rather_than_holding_nothing_out() -> None:
    with pytest.raises(InvalidUnitSplitError, match="set-c"):
        SubsetSplit("set-c").applied_to(STAYS)


def test_a_subset_that_holds_every_unit_leaves_no_training_side() -> None:
    with pytest.raises(InvalidUnitSplitError, match="at least one unit"):
        SubsetSplit("set-a").applied_to(STAYS[:3])


@pytest.mark.parametrize("text", ["", "   ", " set-b", "set-b "])
def test_a_blank_or_padded_subset_is_refused_where_the_policy_is_stated(text: str) -> None:
