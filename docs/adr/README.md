...
| [0029](0029-pretraining-over-a-mixture-of-corpora.md) | Pretraining over a mixture of corpora: one vocabulary chained through the publications, optimiser steps of one corpus each weighed by their count, the best epoch kept by the mean relative validation, and a run that says where it can be picked up from | proposed |
