...
| [synthetic-transfer.md](synthetic-transfer.md) | the transfer leg of the synthetic control at the endpoint's budget, on both pairs: the peaks swept on each pair's own task, the power read off the sweep, then five seeds of every arm at 200 labelled windows read by the registered rules; the coupled pair fails its rule and the null pair its equivalence, both with the pretrained arms worse than training from scratch, while the frozen probe shows the backbone did learn the shared structure — the pipeline's positive control does not pass, and what separates the three explanations is stated |
