...
| [synthetic-transfer.md](synthetic-transfer.md) | the transfer leg of the synthetic control at the endpoint's budget, on both pairs, from failure to diagnosis: at a window of 32 both pairs fail their rules, the ceiling over shared trajectories shows the fault lies above the data, and a window of 128 spanning the factors' periods makes the coupled pair pass; closed at 128 with swept peaks, the coupled pair holds (+0.094, floor 0.053) and the null pair fails its equivalence (+0.025, floor 0.013), which the generator explains — the null shares the family of signals and not the frequencies — and the swapped backbones show that what transfers is the family, with the structure's own share a fifth of the floor and no leak between the pairs: the positive control holds and the null pair is a control of leakage, not of structure |
| [label-efficiency-curve.md](label-efficiency-curve.md) | the first label-efficiency curve, preliminary and on the validation side: the schedule of every arm fixed at one budget before the grid, then the grid of four transfer modes over four budgets under five seeds on the GPU platform, read by the registered rules — the paired interval over engines, the endpoint, the secondary family under Holm, the practical floor — with its one-sentence conclusion and what the curve does not settle; the grid's reading: the endpoint not confirmed, distinguishable but under the practical floor, because the control arm leaves the plateau of the mean predictor under two seeds of five; then the sweep of every arm's peak under the floor of 2,000 steps over three seeds on the platform, where every seed of the control leaves the plateau and the control is the best arm at 200 on the first backbone |
