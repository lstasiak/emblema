...
| [head-and-representation.md](head-and-representation.md) | With the backbone frozen, pooling the last fifth of the window instead of the whole of it takes a linear head from 18.9 % behind the tuned trees to 5.6 % and not distinguishable; the states beat an untrained encoder's everywhere; the last reading alone loses half the trees' accuracy, so the task is read from the movement within the window. |
