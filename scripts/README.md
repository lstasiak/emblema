...
| `classical_baselines_report.py` | measurement | the note on the classical baselines: MiniRocket against aeon's where no draw differs and under aeon's own draws, both fitted on a published corpus under one grid and ridge, and what one cell of every classical method costs; three CSV files, printed as tables in a second step. Run under `uv run --with aeon`, the cost in a process of its own | — |
