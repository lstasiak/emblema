...
def draw(curve: Curve, path: Path, *, caption: str = CAPTION) -> Path:
    """Draw the curve and the comparisons into ``path``."""
    ...
    lower.set_xticklabels(
        [f"{budget}\n({_range(engines[budget])} engines)" for budget in budgets], fontsize=8
    )
    lower.minorticks_off()
    lower.set_xlabel("labelled windows in the budget")
    lower.grid(True, alpha=0.25)

    figure.suptitle(f"Label efficiency on turbofan FD001 — {caption}", fontsize=10)
    figure.tight_layout()
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    print(draw(read(arguments.curve), figure, caption=arguments.caption))
